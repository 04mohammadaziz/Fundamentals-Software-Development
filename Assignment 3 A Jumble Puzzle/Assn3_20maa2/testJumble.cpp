...
void testJumble() {
    ...
    cout << "Invalid difficulty input test:" << endl;
    try {
        JumblePuzzle jp1("COOL", "hardr");
    }
    catch (BadJumbleException& e) {
        cout << e.what() << endl;
    }

    cout << "Long word test:" << endl;
    try {
        JumblePuzzle jp2("PROFESSORJEHADISCOOL", "easy");
    }
    catch (BadJumbleException& e) {
        cout << e.what() << endl;
    }

    cout << "Short word test:" << endl;
    try {
        JumblePuzzle jp3("HA", "easy");
    }
    catch (BadJumbleException& e) {
        cout << e.what() << endl;
    }
